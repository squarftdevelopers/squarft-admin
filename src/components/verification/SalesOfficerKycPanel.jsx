...
import {
  fetchSalesOfficerKycList,
  reviewSalesOfficerKyc,
} from '../../services/salesOfficerKycService';
...
const getOfficerName = (record) =>
  [record?.first_name, record?.last_name].filter(Boolean).join(' ').trim() || 'Sales Officer';
...
export default function SalesOfficerKycPanel() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await fetchSalesOfficerKycList(status);
      setRecords(data);
      setSelectedId((current) => data.some((record) => record.kyc_id === current) ? current : (data[0]?.kyc_id || ''));
    } catch (requestError) {
      setError(requestError?.message || 'Unable to load sales officer KYC records.');
      setRecords([]);
      setSelectedId('');
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    let active = true;
    fetchSalesOfficerKycList(status)
      .then((data) => {
        if (!active) return;
        setRecords(data);
  ...
  const approve = async () => {
    ...
    setError('');
    try {
      await reviewSalesOfficerKyc(selected.kyc_id, { status: 'verified' });
      setLoading(true);
      setStatus('verified');
    } catch (requestError) {
  ...
  const reject = async () => {
    if (!selected) return;
    const reason = await prompt('Enter the rejection reason for this sales officer KYC.', { title: 'Reject KYC' });
    if (!reason?.trim()) return;

    setActionLoading(true);
    setError('');
    try {
      await reviewSalesOfficerKyc(selected.kyc_id, {
        status: 'rejected',
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm lg:col-span-1">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            <div>
              <h4 className="text-xs font-black uppercase tracking-[0.1em] text-gray-900">Sales Officer KYC</h4>
          ...
          <div className="mt-3 max-h-[520px] space-y-2 overflow-y-auto pr-1">
            {loading ? (
              ...
              <div className="rounded-lg border border-dashed border-gray-200 bg-gray-50 p-6 text-center text-xs font-bold text-gray-500">
                No sales officer KYC records in this stage.
              </div>
            ) : records.map((record) => (
              <button
                key={record.kyc_id}
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm lg:col-span-2">
          {!selected ? (
            <div className="flex min-h-72 items-center justify-center rounded-lg border border-dashed border-gray-200 bg-gray-50 p-8 text-center">
              <p className="text-xs font-bold text-gray-500">Select a sales officer KYC record to review.</p>
            </div>
          ) : (
            <>
